...
static struct plog __plog = {
	.level = -1,
};

inline void
_format_time(char* time_buff) {
	struct timeval tv;
...
static inline void
_append(struct plog_buffer* plog_buffer, const char* msg, int _len) {
...
static void
_destroy_plog_buffers(struct plog_buffer** _arr) {
	int i;
	for (i = 0; i < N_ELEMENTS; i++) {
		free(_arr[i]);
	}
...
int
plog_open(const char* logname, int level, long rotate_size) {
	__plog.lock = 0;
	__plog.level = level;
	...
	if (pthread_create(&__plog.tid, NULL, thread_flush, NULL) < 0) {
...
void
plog_printf(int level, const char *fmt, ...) {
